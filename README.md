...
1. Run `Dataset_stratification_and_merge.ipynb` to merge the two Roboflow datasets into a single stratified dataset (you will need a Roboflow API key).
2. Run `Dataset_Training.ipynb` to train YOLO11m and produce a `best.pt` checkpoint.
...
- Raw CCTV footage and trained model weights are not included in this repository due to size and licensing.
